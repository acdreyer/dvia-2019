...
print(maxy1)
  ...
  for (let i = 0; i < tabinput.getRowCount(); i++) {
    ...
    //     var pal2 = Brewer.sequential('YlGn', Infinity, 0, miny1, maxy1) 
    // for (var k=0; k<(map(value,0,maxy1,0,plotheight)); k++){
    //     noStroke()
    //     fill(pal2.colorForValue(k))
    //     rect(x + offset * barwidth,k, barwidth,-(k+1))
    // }
...
function getpalette(tabinput, colNames, colorstring, numberOfShades) {
  ...
  for (var r = 0; r < tabinput.getRowCount(); r++) {
    ...
    colNames.forEach(function(value, index) {
      ...
      keepvalue = tabinput.getNum(r, index)
    });
...
// ---------------------------Plot bottom---------------------------
  ...
  textSize(16); stroke(255);  fill(255);
  text('Yield at Height of Burst', (xd + ww / 2), ye-hh - ypadding1)
  text('Year of test', (xd + ww / 2), ye + ypadding1*4)
  xaxislabel(table, "WorldExternal", xx, yy + ypadding1, ww, hh, 12, 0, 1, '#fff',5)
  yaxislabel('Height of Burst (0.5 km)', xx, yy, ww, hh, 12)
  //         data   value            x  y  w  h  scl       or  strW  str   fill   palette        
  var palette = getpalette(table, ["YIELD","YIELD"], "YlOrRd", 9);
  ...
  bubblePlot(below3000m, "YEAR","HOB","YIELD","MON_NUM", xx, yy, ww, hh, 1    , yscale ,szescale, 0,    0.2,      '#777',   palette)
...
// ---------------------------Plot middle---------------------------
  ...
  xaxislabel(table, "Years", xx, yy+ypadding1, ww, hh, 12, 0, 1, '#fff',5)
  ...
  linegraph(table,"WorldTotal",xx,yy,ww,hh,bb,0,1,'#fff')
...
// ---------------------------Plot left---------------------------
  ...
  //         data   value            x  y  w  h  scl       or  strW  str   fill   palette        
  var palette = getpalette(table, ["Yield", "Yield"], "Greys", 9);
  //    (tabinput,             colName,               x,  y, wdth, hgt, zerorat   scal    scal,  offset,strkWght, strkClr, palette)
...
// ---------------------------------------Plot right
  ...
  textStyle(NORMAL); textAlign(CENTER);   
  textSize(16); stroke(255);  fill(0);
  text('Radiation Comparison', (xf + ww / 2), yd - hh - ypadding1)
  textStyle(NORMAL); textAlign(RIGHT); 
...
text(t, canvaslx/2, ypadding1*3 );  // Text wraps within text box
